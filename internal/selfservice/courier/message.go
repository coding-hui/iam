...
// Message represents a courier message/event.
type Message struct {
...
// Courier defines the interface for courier operations.
...
// courierImpl implements Courier.
type courierImpl struct {
	mu    sync.Mutex
	hooks map[string][]string // eventType -> targetURLs
}

// NewCourier creates a new courier.
func NewCourier() *courierImpl {
	return &courierImpl{
...
func (m *courierImpl) RegisterHook(ctx context.Context, eventType, targetURL string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *courierImpl) UnregisterHook(ctx context.Context, eventType, targetURL string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *courierImpl) SendEvent(ctx context.Context, eventType string, payload any) error {
	m.mu.Lock()
...
// Ensure courierImpl implements Courier.
var _ Courier = (*courierImpl)(nil)
