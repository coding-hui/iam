...
function iam::release::package_tarballs() {
  ...
  iam::util::wait-for-jobs || {
    iam::log::error "previous tarball phase failed"
    return 1
  }
...
function iam::release::updload_tarballs() {
  ...
  for file in $(ls ${RELEASE_TARS}/*); do
...
function iam::release::package_server_tarballs() {
  ...
  iam::util::wait-for-jobs || {
    iam::log::error "server tarball creation failed"
    exit 1
  }
}
...
function iam::release::package_client_tarballs() {
  ...
  iam::log::status "Waiting on tarballs"
  iam::util::wait-for-jobs || {
    iam::log::error "client tarball creation failed"
    exit 1
  }
}
...
EOF
    ...
    iam::util::wait-for-jobs || {
      iam::log::error "previous Docker build failed"
      return 1
    }
